...
export function HistoryPanel(): JSX.Element {
  ...
  const today = todayId()
  // Always show today at the top even if it has no items yet
  ...
  return (
    // Slide in/out by animating width. overflow-hidden clips content during transition.
    <aside
      className={`flex h-full flex-col border-l border-stone-100 bg-white transition-all duration-200 overflow-hidden ${
        sidebarOpen ? 'w-48 min-w-[12rem]' : 'w-0 min-w-0'
      }`}
    >
